...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        spaces_moved = 0
        spot = self.place
        while not spot.is_hive:
            if self.lower_bound <= spaces_moved <= self.upper_bound and spot.bees:
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, gamestate):
        ...
        while spot:
            if spot.ant:
                spot.ant.double()
                if spot.ant.is_container and spot.ant.ant_contained:
                    spot.ant.ant_contained.double()
            spot = spot.exit
        super().action(gamestate)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        def slowed_bee(gamestate):
            if target.slow_turns_left == 0:
                target.action = previous_action
                previous_action(gamestate)
            elif gamestate.time % 2 == 1:
                target.slow_turns_left -= 1
            else:
                type(target).action(target, gamestate)
                target.slow_turns_left -= 1
        target.action = slowed_bee
